...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db_component_object = DBComponentSource(all_db_components, self.Session())
        self.comp_manager = ComponentManager([self.db_component_object, self.component_object])
        self.node_factory = NodeFactoryDB(self.comp_manager)
        
        starting_room = self.node_factory.create_new_node({"container":{}})
        
        self.avatar_factory = AvatarFactory(self.node_factory, self.comp_manager, data={"starting_room":starting_room.id, "starting_location":"temp", "player_id":"temp"})
    ...
    def testCreate(self):
        ...
        self.account_utils.make_account("foo", "bar", "foobar@foobar.com", session)
        session.close()
        
    def testCreateAvatar(self):
        session = self.Session()
        acc = self.account_utils.make_account("test", "test1", "test@test.com", session)
    ...
    def testGetAvatar(self):
        session = self.Session()
        acc = self.account_utils.make_account("test", "test2", "test@test.com", session)
