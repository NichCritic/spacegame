...
class AttachSystem(System):
    ...
    def handle(self, node):
        parent_node = self.node_factory.create_node(node.attached.target_id, ["position", "rotation"])
        ...
        node.position.x = (node.attached.x * math.cos(p_rot) - node.attached.y * math.sin(p_rot)) + p_x
        node.position.y = (node.attached.x * math.sin(p_rot) + node.attached.y * math.cos(p_rot)) + p_y
        node.rotation.rotation = p_rot + node.attached.rotation

        logging.info(f"Gun at {node.position.x},{node.position.y}, {node.rotation.rotation}")